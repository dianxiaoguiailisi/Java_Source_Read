...
/**
 * 通用 ApplicationContext 实现，包含单个内部
 * {@link org.springframework.beans.factory.support.DefaultListableBeanFactory}
 * 实例，并且不采用特定的 bean 定义格式。实现
 * {@link org.springframework.beans.factory.support.BeanDefinitionRegistry}
 * 接口，以便允许对其应用任何 bean 定义读取器。
 *
 * <p>典型用法是通过
 * {@link org.springframework.beans.factory.support.BeanDefinitionRegistry}
 * 接口，然后调用 {@link #refresh（）} 来初始化这些 bean
 * 使用应用程序上下文语义（处理
 * {@link org.springframework.context.ApplicationContextAware}，自动检测
 * {@link org.springframework.beans.factory.config.BeanFactoryPostProcessor BeanFactoryPostProcessors}，
 * 等）。
 *
 * 与其他 <p>ApplicationContext 实现相比，这些实现创建新的
 * 每次刷新的内部 BeanFactory 实例，内部 BeanFactory 的
 * 此上下文从一开始就可用，以便能够注册 Bean
 * 定义。{@link #refresh（）} 只能调用一次。
 *
 * <p>使用示例：
 *
 * <pre class=“代码”>
 * GenericApplicationContext ctx = 新的 GenericApplicationContext（）;
 * XmlBeanDefinitionReader xmlReader = 新 XmlBeanDefinitionReader（ctx）;
 * xmlReader.loadBeanDefinitions（新 ClassPathResource（“applicationContext.xml”））;
 * 属性BeanDefinitionReader propReader = 新属性BeanDefinitionReader（ctx）;
 * propReader.loadBeanDefinitions（新 ClassPathResource（“otherBeans.properties”））;
 * ctx.refresh（）;
 *
 * MyBean myBean = （我的Bean） ctx.getBean（“我的Bean”）;
 * ...</pre>
 *
 * 对于 XML bean 定义的典型情况，只需使用
 * {@link ClassPathXmlApplicationContext} 或 {@link FileSystemXmlApplicationContext}，
 * 更容易设置 - 但灵活性较差，因为您可以只使用标准
 * XML Bean 定义的资源位置，而不是混合任意 Bean
 * 定义格式。Web 环境中的等效项是
 * {@link org.springframework.web.context.support.XmlWebApplicationContext}。
 *
 * <p>对于应该读取的自定义应用程序上下文实现
 * 以可刷新的方式进行特殊的 bean 定义格式，考虑派生
 * 来自 {@link AbstractRefreshableApplicationContext} 基类。
 *
...
public class GenericApplicationContext extends AbstractApplicationContext implements BeanDefinitionRegistry {
	//GenericApplicationContext组合DefaultListableBeanFactory档案馆
